...
1. 启动后窗口**立即显示品牌加载页**（蓝鲸 + 进度提示），本地服务自动在随机空闲端口启动，就绪后自动进入主界面
...
- **主要耗时**：dsh 需加载 200+ 插件（Node 冷启动 + 逐模块 require），本机热启动实测约 **20 秒**
- **Windows 明显变慢（1–2 分钟）的原因**：① 便携版每次启动需自解压 140MB+ 运行时（约 2.5 万个文件）；② 杀毒软件实时扫描这些文件
- **建议**：追求最快启动请使用**安装版**；若便携版/安装版启动仍明显偏慢，将应用目录加入杀毒软件排除列表（Defender →「病毒和威胁防护」→「管理设置」→「排除项」）
- 启动期间窗口**不会黑屏**：先显示品牌加载页，服务就绪后自动切换主界面
