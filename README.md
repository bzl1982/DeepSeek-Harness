...
3. 启动后窗口**立即显示品牌加载页**（蓝鲸 + 进度提示），本地服务就绪后自动进入主界面；
   首次启动需加载 200+ 插件，约 20–40 秒（受磁盘与杀毒软件影响可能更久，见「启动性能」）
...
- **模型列表**：设置 → 模型选择中内置全部 DeepSeek 官方模型选项：
  `deepseek-flash`（**DeepSeek-V4.1-Flash**，2026-09-10 发布，原生多模态，1M 上下文）、
  `deepseek-v4-pro`（V4 Pro）、`deepseek-v4-flash`、`deepseek-v4-flash-vision-exp`
  （旧名自动路由到 V4.1 Flash，按 Flash 价计费）
- **会话结算**：每次会话完成后，消息流末尾自动显示「本次会话结算」卡片：
  调用模型、插件/工具调用（种类与次数）、Token 总量（未缓存输入 / 缓存读取 / 缓存写入 / 输出含推理）、
  缓存命中率、**费用估算**（按官方价格表）与**计价时段**（高峰/闲时）
- **本轮用量费用**：每条助手消息的「本轮用量」面板新增「费用估算」行（金额 + 高峰/闲时 + 模型）
- 高峰时段 = 北京时间周一至五 9:00-12:00、14:00-18:00；价格为估算，以 DeepSeek 官方账单为准
...
- **主要耗时**：dsh 需加载 200+ 插件（Node 冷启动 + 逐模块 require），本机热启动实测约 **20 秒**；冷启动（重启后首次）会慢一些
- **Windows 显著变慢（1–2 分钟）的常见原因**：杀毒软件（Windows Defender 等）实时扫描应用目录下 2.5 万+ 文件，每次模块加载都会被扫描
- **建议**：启动明显偏慢时，可将应用安装目录加入杀毒软件排除列表（Defender →「病毒和威胁防护」→「管理设置」→「排除项」→ 添加应用所在目录），排除后启动恢复 20 秒级别
- 启动期间窗口**不会黑屏**：应用先显示品牌加载页，服务就绪后自动切换主界面
